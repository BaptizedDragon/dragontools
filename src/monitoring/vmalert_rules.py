"""Fixed rule API validation including annotation-only host event details."""
HOST_EVENT_RULES = {
  "groups": [
    {
...
def validate(value, kind, now=None):
    ...
    expected_groups = {"dragontools-logs", "dragontools-host-events"} if kind == "logs" else {"dragontools-probes", "dragontools-hosts"}
...
def validate_group(group, kind, file, datasource, now):
    ...
    if group['name'] == 'dragontools-host-events':
        spec = HOST_EVENT_RULES['groups'][0]
        # Shared fixed rule specification: package/kernel data lives only in
        # annotation queries, never in the primary alert result/ALERTS labels.
        validate_application_group(group, file, spec, now)
        return
    annotations = {
...
def validate_application_group(group, file, expected, now):
    require(group.get("file") == file and group.get("type") == expected["type"] and group.get("interval") == (60 if expected["interval"] == "1m" else 30))
    require(not group.get("params") and not group.get("headers") and not group.get("notifier_headers") and not group.get("labels"))
