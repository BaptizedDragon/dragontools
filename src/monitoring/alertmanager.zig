...
pub const telegram_template = @embedFile("telegram.tmpl");
pub const template_path = "/etc/dragontools/alertmanager/templates/telegram.tmpl";
pub fn templateCommand(a: std.mem.Allocator, mode: []const u8) !remote.Input {
    return .{ .command = try remote.shell(a, &.{ "python3", "-I", "-B", "-c", @embedFile("telegram_template.py"), mode }), .bytes = telegram_template };
}
...
pub const enabled_config =
    \\# Managed by DragonTools
    \\templates: ['/etc/dragontools/alertmanager/templates/telegram.tmpl']
    \\global:
    \\  resolve_timeout: 5m
    \\route:
    \\  receiver: discard
    ...
    \\  repeat_interval: 4h
    \\  routes:
    \\    - receiver: telegram-host-events
    \\      matchers: ['source="host-maintenance"', 'alertname=~"HostRebootRequired|HostRebootRequirementCleared"']
    \\      group_by: [alertname, host, event_id]
    \\      group_wait: 0s
    \\      group_interval: 1m
    \\      repeat_interval: 24h
    \\    - receiver: telegram
    ...
    \\        send_resolved: true
    \\        parse_mode: HTML
    \\        message: '{{ template "dragontools.telegram.message" . }}'
    \\  - name: telegram-host-events
    \\    telegram_configs:
    \\      - bot_token_file: /etc/dragontools/alertmanager/secrets/telegram-bot-token
    \\        chat_id_file: /etc/dragontools/alertmanager/secrets/telegram-chat-id
    \\        send_resolved: false
    ...
    \\
;
pub const unit_text = std.fmt.comptimePrint(
    \\# Managed by DragonTools
    \\[Unit]
    ...
    \\ExecStart=/opt/dragontools/components/alertmanager/current/alertmanager --config.file=/etc/dragontools/alertmanager/alertmanager.yml --storage.path=/var/lib/dragontools/alertmanager --web.listen-address={s} --cluster.listen-address= --log.level=info
    \\Restart=on-failure
    \\RestartSec=5s
    \\TimeoutStopSec=60s
    \\NoNewPrivileges=yes
...
pub fn install(a: std.mem.Allocator, r: remote.Remote, report: *workflow.Report, arch: host.Arch) !void {
    ...
    if (report.telegram_configured) {
        const template = try templateCommand(a, "install");
        defer a.free(template.command);
        _ = try report.call(r, .config, template);
    }
...
pub fn health(a: std.mem.Allocator, r: remote.Remote, report: *workflow.Report, arch: host.Arch) !void {
    ...
    if (std.mem.eql(u8, mode, "enabled")) {
        const template = try templateCommand(a, "verify");
        defer a.free(template.command);
        _ = try readiness.deterministic(a, r, report, .managed_state, template);
