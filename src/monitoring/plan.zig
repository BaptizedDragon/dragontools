...
pub const unavailable = "Not yet available: Grafana, vmalert, Alertmanager, Telegram, Vector, vmagent, OTel Collector, monitoring agents, firewall, TLS, update monitoring and maintenance.\n";

pub fn render(a: std.mem.Allocator) ![]const u8 {
    return std.fmt.allocPrint(a, "Plan: install VictoriaMetrics, VictoriaLogs and VictoriaTraces.\n" ++
...
test "installation plan describes three pinned components and the remaining boundary" {
    const output = try render(std.testing.allocator);
    defer std.testing.allocator.free(output);
    for ([_][]const u8{ "VictoriaMetrics: loopback:8428", "VictoriaLogs: loopback:9428", "VictoriaTraces: loopback:10428", vm.version, vl.version, vt.version, "retention: 90d; filesystem reserve: 20%", "logical limit: 100y", "partition budget: 75%", "newest two partitions", "No remote operations performed." }) |expected| {
