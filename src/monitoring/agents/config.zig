...
test "Vector selects only explicit journal services with trusted identity and bounded mTLS forwarding" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, no_logs, "type: journald") == null);
    try std.testing.expect(std.mem.indexOf(u8, no_logs, "type: demo_logs") == null);
}
...
test "application agents merge scopes with trusted labels and metrics-only edits isolate vmagent" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, host_only, "type: journald") == null);
