//! Concrete shared lifecycle fragments for the two agents and ingestion service.
const std = @import("std");
const remote = @import("../../system/remote.zig");
pub const Kind = enum { vector, vmagent, ingestion };
pub fn owner(kind: Kind) []const u8 {
    return if (kind == .ingestion) "dt-ingest" else if (kind == .vector) "dt-vector" else "dt-vmagent";
}
pub fn marker(a: std.mem.Allocator, kind: Kind) ![]const u8 {
    return std.fmt.allocPrint(a, "/var/lib/dragontools/{s}-restart-required", .{@tagName(kind)});
}
pub fn unitPath(a: std.mem.Allocator, kind: Kind) ![]const u8 {
    return std.fmt.allocPrint(a, "/etc/systemd/system/dragontools-{s}.service", .{@tagName(kind)});
}
pub fn preflight(a: std.mem.Allocator, kind: Kind) ![]const u8 {
    return remote.shell(a, &.{
        ...
        \\  test "$(getent passwd "$user" | cut -d: -f6)" = /var/lib/dragontools/$name || exit 41
        ...
        \\  useradd --system --user-group --home-dir /var/lib/dragontools/$name --no-create-home --shell /usr/sbin/nologin "$user"
        \\  printf changed
        \\fi
        ,
        "dragontools-agent-account", @tagName(kind), owner(kind),
    });
}
...
pub fn activation(kind: Kind) []const u8 {
    return switch (kind) {
        ...
        .ingestion => @import("../install.zig").activation("ingestion"),
    };
}
