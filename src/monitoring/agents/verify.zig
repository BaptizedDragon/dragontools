...
pub fn serviceSpec(a: std.mem.Allocator, kind: common.Kind, arch: host.Arch, command: []const u8, file: File) ![]const u8 {
    ...
    return std.json.Stringify.valueAlloc(a, .{
        ...
        .command = command,
        .files = &files,
        .ingress_registry_mode = @import("../../agent/files.zig").registry_mode,
        .ingress_server_mode = @import("../../agent/files.zig").server_mode,
        .ingress_private_directory_mode = @import("../../agent/files.zig").private_directory_mode,
...
/// Station and application entrypoints share the same fixed, read-only checks.
pub fn managed(a: std.mem.Allocator, r: remote.Remote, report: *@import("../install.zig").Report, kind: common.Kind, data: []const u8) !void {
    if (kind != .ingestion and kind != .caddy) {
        _ = try ready.deterministic(a, r, report, .managed_state, try common.python(a, ingress.checks_program, &.{ "managed", data }));
        return;
    }
    const checks: []const ready.Check = if (kind == .caddy)
        &.{ .caddy_account, .caddy_binary, .caddy_unit, .caddy_config, .caddy_directories, .caddy_systemd_properties, .caddy_credentials }
    else
        &.{ .managed_account, .managed_unit, .managed_helper, .managed_directories, .managed_registry, .managed_server_state, .managed_systemd_properties };
    for (checks) |check| {
        _ = try ready.deterministic(a, r, report, check, try common.python(a, ingress.checks_program, &.{ @tagName(check), data }));
    }
}
pub fn service(a: std.mem.Allocator, r: remote.Remote, report: *model.Report, registration: model.Registration, arch: host.Arch, kind: common.Kind) !void {
    ...
    try managed(a, r, &report.state, kind, data);
    ...
    if (kind == .vector or kind == .vmagent) {
        ...
        if (kind == .vector) try endpointSignal(a, r, report, registration, "vector", .logs, guard);
    }
}
pub const network_guidance = "DragonTools does not manage DNS or provider firewalls. Ensure the station hostname resolves and TCP 9443 (metrics) and TCP 9444 (host events and selected logs) are allowed.\n";
...
pub fn enrollmentEndpoints(a: std.mem.Allocator, r: remote.Remote, report: *model.Report, registration: model.Registration, kind: []const u8) !void {
    ...
    if (!std.mem.eql(u8, kind, "vmagent")) try endpointSignal(a, r, report, registration, kind, .logs, null);
}
fn endpointSignal(a: std.mem.Allocator, r: remote.Remote, report: *model.Report, registration: model.Registration, kind: []const u8, signal: ingress.Signal, guard: ?[]const u8) !void {
    secureEndpointGuarded(a, r, report, try ingress.endpointForSignal(a, registration.station, kind, registration.host, signal), guard, if (std.mem.eql(u8, kind, "vmagent")) .metrics_mtls_authenticated else .secure_endpoint) catch |err| {
...
pub fn secureEndpoint(a: std.mem.Allocator, r: remote.Remote, report: *model.Report, command: anytype) !void {
    return secureEndpointGuarded(a, r, report, command, null, .secure_endpoint);
}
fn secureEndpointGuarded(a: std.mem.Allocator, r: remote.Remote, report: *model.Report, command: anytype, runtime_guard: ?[]const u8, check: ready.Check) !void {
    const Probe = struct {
    ...
    var probe = Probe{ .source = r, .report = report, .runtime_guard = runtime_guard, .check = check };
    ...
    try ready.poll(a, adapted, &report.state, check, ready.http_ms, command, ready.ready);
}
...
test "native endpoint guard and TLS probe share one deadline and deterministic guard failures do not retry" {
    ...
    for (0..3) |scenario| {
        ...
        const result = secureEndpointGuarded(std.testing.allocator, fake.remoteValue(), &report, request, "fixed guard", .secure_endpoint);
        switch (scenario) {
            0 => try result,
...
pub fn signals(a: std.mem.Allocator, app: remote.Remote, station: remote.Remote, report: *model.Report, registration: model.Registration, comptime mode: []const u8) !void {
    ...
    const selected = try registration.selected(report.application).json(a);
    if (std.mem.eql(u8, mode, "app")) {
        for ([_]ready.Check{ .metrics_agent_active, .metrics_source_ready, .metrics_station_reachable }) |check| {
            const budget: u32 = if (check == .metrics_agent_active) ready.active_ms else if (check == .metrics_source_ready) ready.telemetry_ms else ready.http_ms;
            try ready.poll(a, app, &report.state, check, budget, try common.python(a, @embedFile("metrics_checks.py"), &.{ @tagName(check), selected, started }), ready.ready);
        }
    ...
    const check: ready.Check = if (std.mem.eql(u8, mode, "host")) .host_metrics_ready else if (std.mem.eql(u8, mode, "logs")) .log_stream_ready else .application_metrics_visible;
