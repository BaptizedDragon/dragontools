...
pub const Component = enum {
    ...
    pub fn name(self: Component) []const u8 {
        return switch (self) {
            ...
            .ingress_auth => "Ingress authorization",
            .caddy => "Caddy mTLS ingress",
    ...
    pub fn text(self: Event) []const u8 {
        return switch (self.phase) {
            .component_started => switch (self.component) {
                .agent_helper => "DragonTool agent helper\n",
                .ingress_auth => "[9/10] Ingress authorization\n",
                .caddy => "[10/10] Caddy mTLS ingress\n",
                .victoriametrics => if (self.station_enabled) "[1/10] VictoriaMetrics\n" else "[1/4] VictoriaMetrics\n",
                .victorialogs => if (self.station_enabled) "[2/10] VictoriaLogs\n" else "[2/4] VictoriaLogs\n",
                .victoriatraces => if (self.station_enabled) "[3/10] VictoriaTraces\n" else "[3/4] VictoriaTraces\n",
                .grafana => if (self.station_enabled) "[4/10] Grafana\n" else "[4/4] Grafana\n",
                .blackbox_exporter => "[5/10] Blackbox exporter\n",
                .alertmanager => "[6/10] Alertmanager\n",
                .vmalert_logs => "[7/10] vmalert logs\n",
                .vmalert_metrics => "[8/10] vmalert metrics\n",
            },
