...
test "Alertmanager supported Telegram file refs and hardening keep both values out of ordinary configuration" {
    for ([_][]const u8{ "bot_token_file:", "chat_id_file:", "send_resolved: true", "warning|critical" }) |field| try std.testing.expect(std.mem.indexOf(u8, am.enabled_config, field) != null);
