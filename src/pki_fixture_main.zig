...
fn run(init: std.process.Init) !void {
    ...
    if (args.len == 2 and (std.mem.eql(u8, args[1], "ingress-checks") or std.mem.eql(u8, args[1], "caddy-checks"))) {
        ...
        const output = try std.json.Stringify.valueAlloc(a, .{
            .spec = try @import("monitoring/ingress.zig").spec(a, kind, .amd64),
            .activate = common.activation(kind),
            .finalize = try common.finalize(a, kind),
        }, .{});
        try std.Io.File.stdout().writeStreamingAll(init.io, output);
    } else if (args.len == 2 and std.mem.eql(u8, args[1], "helper-scripts")) {
        ...
        const dir = try events.open(init.io, root);
        defer dir.close(init.io);
        if (std.mem.eql(u8, args[3], "verify")) {
            if (try events.load(a, init.io, dir) == null) return error.InvalidFixture;
        } else {
            var buffer: [8192]u8 = undefined;
            var output = std.Io.File.stdout().writer(init.io, &buffer);
            try events.check(a, init.io, dir, try events.observe(a, init.io, root), &output.interface);
        }
