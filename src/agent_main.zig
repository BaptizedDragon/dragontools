...
fn entry(init: std.process.Init, enabled: *bool, stage: *diagnostics.Stage) !u8 {
    ...
    if (args.len == 3 and std.mem.eql(u8, args[1], "maintenance")) {
        if (std.mem.eql(u8, args[2], "events") or std.mem.eql(u8, args[2], "events-verify")) {
            try @import("maintenance/events.zig").run(a, init.io, std.mem.eql(u8, args[2], "events-verify"));
            return 0;
        }
