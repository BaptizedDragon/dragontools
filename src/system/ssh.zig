...
pub const Ssh = struct {
    ...
    fn connectionArgs(self: *Ssh) !std.ArrayList([]const u8) {
    ...
    /// An attached, private master clears configured forwards before any socket
    /// opens. Explicit forwards are added later through its acknowledged mux API.
    /// Normal user config still owns authentication, ProxyJump, user and port.
    pub fn tunnelArgv(self: *Ssh, control_path: []const u8) ![]const []const u8 {
        var args = try self.connectionArgs();
        try args.appendSlice(self.allocator, &.{ "-N", "-S", control_path, "-o", "ControlMaster=yes", "-o", "ControlPersist=no", "-o", "ForkAfterAuthentication=no", "-o", "ExitOnForwardFailure=yes", "-o", "GatewayPorts=no", "-o", "PermitLocalCommand=no", "-o", "RemoteCommand=none", "--", self.options.ssh_host orelse self.options.host });
