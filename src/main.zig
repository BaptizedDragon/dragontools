...
fn execute(init: std.process.Init, options: cli.Options) !void {
    ...
    if (@import("cli/spec.zig").uiCommand(options.command)) {
        try @import("monitoring/ui.zig").run(a, init.io, options);
        return;
    }
    ...
    var report: @import("monitoring/install.zig").Report = .{ .station_enabled = true, .ingress_hostname = options.ingress_hostname, .probes = options.probes, .telegram_credentials = telegram, .telegram_configured = options.telegram_bot_token_op != null, .grafana_credentials = credentials, .progress = .{ .context = &progress_output, .write = ProgressOutput.write } };
    switch (options.command) {
        .install, .verify => {
            print(init.io, if (options.command == .install) "Installing the monitoring station over SSH (ten services)...\n" else "Verifying the monitoring station over SSH (ten services)...\n");
            ...
            print(init.io, "Caddy mTLS ingress: IPv4 0.0.0.0:9443 metrics / 0.0.0.0:9444 logs\n  healthy; client certificates required; zero registered clients is valid\nIngress authorization: private Unix sockets only\n");
        },
        .status => {
...
test {
    ...
    _ = @import("monitoring/agents/vector_config.zig");
