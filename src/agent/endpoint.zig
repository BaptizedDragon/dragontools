...
pub fn check(ctx: s.Context, endpoint: []const u8, kind: []const u8, host: []const u8) !void {
    ...
    var fd: c_int = -1;
    const result = dragontools_connect(try ctx.store.a.dupeZ(u8, endpoint), "9443", &fd);
    ...
    try @import("../pki/tls.zig").health(ctx.store.a, fd, ca, cert, key, endpoint, host, ctx.now);
}
