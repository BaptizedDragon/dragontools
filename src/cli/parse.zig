...
pub fn parse(a: std.mem.Allocator, args: []const []const u8) !Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (item.kind == .boolean) {
            if ((eq(key, "--open") and seen.contains("--no-open")) or (eq(key, "--no-open") and seen.contains("--open"))) return error.ConflictingBrowserOptions;
            if (eq(key, "--no-open")) {
                o.open_browser = false;
                continue;
            }
            if (eq(key, "--open")) o.open_browser = true else if (eq(key, "--json")) o.json = true else if (eq(key, "--help")) o.help = true else if (eq(key, "--plan")) o.plan = true else if (eq(key, "--set-default-shell")) o.set_default_shell = true else if (eq(key, "--update-managed-zshrc")) o.update_managed_zshrc = true;
            continue;
        }
...
test "UI command parsing shares station config with explicit replacement and no TLS target inference" {
    const a = std.testing.allocator;
    const io = std.testing.io;
    var temp = std.testing.tmpDir(.{});
    defer temp.cleanup();
    for ([_][]const u8{ "metrics", "logs", "traces", "grafana", "alerts", "log-alerts", "alertmanager" }) |name| {
        var o = try parse(a, &.{ "monitoring", "ui", name });
        defer o.deinit(a);
        try std.testing.expect(spec.uiCommand(o.command));
        try std.testing.expect(o.open_browser);
        var manual = try parse(a, &.{ "monitoring", "ui", name, "--no-open" });
        defer manual.deinit(a);
        try std.testing.expect(!manual.open_browser);
        var explicit_open = try parse(a, &.{ "monitoring", "ui", name, "--open" });
        defer explicit_open.deinit(a);
        try std.testing.expect(explicit_open.open_browser);
        try std.testing.expectError(error.StationConfigurationRequired, loadStation(a, io, &o, temp.dir));
        try mergeText(a, &o, "version=1\n[connection]\nssh_host='station-alias'\n[station]\nhostname='different-tls.example'\n[grafana]\nusername={op='op://Fixture/Grafana/user'}\npassword={op='op://Fixture/Grafana/password'}\n");
        try std.testing.expectEqualStrings("station-alias", o.ssh_host.?);
        try std.testing.expectEqualStrings("different-tls.example", o.ingress_hostname.?);
        try std.testing.expectError(error.UnknownFlag, parse(a, &.{ "monitoring", "ui", name, "--remote-port", "9999" }));
        try std.testing.expectError(error.FlagNotAllowed, parse(a, &.{ "monitoring", "ui", name, "--plan" }));
        try std.testing.expectError(error.DuplicateFlag, parse(a, &.{ "monitoring", "ui", name, "--open", "--open" }));
        try std.testing.expectError(error.DuplicateFlag, parse(a, &.{ "monitoring", "ui", name, "--no-open", "--no-open" }));
        try std.testing.expectError(error.ConflictingBrowserOptions, parse(a, &.{ "monitoring", "ui", name, "--open", "--no-open" }));
        try std.testing.expectError(error.ConflictingBrowserOptions, parse(a, &.{ "monitoring", "ui", name, "--no-open", "--open" }));
        try std.testing.expectError(error.InvalidSshHost, parse(a, &.{ "monitoring", "ui", name, "--ssh-host", "x;touch /tmp/not-executed" }));
    }
    try temp.dir.writeFile(io, .{ .sub_path = config.default_path, .data = "invalid-default" });
    try temp.dir.writeFile(io, .{ .sub_path = "other.toml", .data = "version=1\n[connection]\nssh_host='file-alias'\n" });
    var explicit = try parse(a, &.{ "monitoring", "ui", "logs", "--config", "other.toml", "--ssh-host", "override-alias" });
    defer explicit.deinit(a);
    try loadStation(a, io, &explicit, temp.dir);
    try std.testing.expectEqualStrings("override-alias", explicit.ssh_host.?);
    var tls_only = try parse(a, &.{ "monitoring", "ui", "traces" });
    defer tls_only.deinit(a);
    try std.testing.expectError(error.HostRequired, mergeText(a, &tls_only, "version=1\n[station]\nhostname='must-not-be-ssh.example'\n"));
    try std.testing.expectError(error.FlagNotAllowed, parse(a, &.{ "monitoring", "status", "--open" }));
    try std.testing.expectError(error.FlagNotAllowed, parse(a, &.{ "monitoring", "status", "--no-open" }));
}
