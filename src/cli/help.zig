...
pub fn render(a: std.mem.Allocator, node: spec.Node) ![]const u8 {
    ...
    if (item.command) |command| {
        ...
        if (spec.stationCommand(command) and !spec.uiCommand(command)) try w.writeAll("\nLoads optional ./station.toml from CWD; --config selects a different file.\nNo parent/home/XDG search. Missing default permits CLI-only usage. Version 1\nsupports connection.ssh_host, station.hostname, Grafana/Telegram secret references\nand named probes. Deprecated [ingress].hostname is accepted unless conflicting.\nRelative config paths are allowed. Literal credentials and unknown keys fail.\nCLI values override config; --host replaces the configured SSH alias and\n--ingress-hostname overrides station.hostname. Credential references are paired.\nHelp, completion, status and --plan never resolve secrets; install and verify\nresolve Grafana references locally; Telegram resolves only during install.\nWithout references, Grafana administrator credentials remain unmanaged.\n");
    }
    try w.writeAll("\n  --help\n      Show help for this command.\n");

    if (node == .ui or (item.command != null and spec.uiCommand(item.command.?))) {
        try w.writeAll(
            \\
            \\Uses ./station.toml from CWD, or one explicit --config PATH. CLI connection
            \\options override the file; the station TLS hostname never selects SSH.
            \\No parent/home/XDG search. Without a file, provide --ssh-host or --host.
            \\VMUI is the primary investigation UI; Grafana remains available for dashboards.
            \\alerts/log-alerts show rule definitions, evaluation and pending/firing state.
            \\alertmanager shows active alerts, silences, grouping and notification routing.
            \\Local forwarding binds only 127.0.0.1, preferring the component's own port
            \\and choosing a free port automatically if busy. Prints the actual local URL.
            \\The tunnel stays attached until Ctrl-C. No background mode or remote changes.
            \\Waits up to 10 seconds for the forwarded UI, then opens a browser automatically.
            \\--no-open suppresses the browser; opener failure leaves the tunnel active.
            \\No Grafana/Telegram secret references are resolved. Grafana keeps local login.
            \\VictoriaTraces VMUI is available; application tracing agents remain unavailable.
            \\
        );
    ...
    try w.writeAll(
        ...
        \\OTel traces agents, firewall, TLS and legacy Telegram flags are unavailable.
