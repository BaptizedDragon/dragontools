...
test "shell completion encodes the nested tree and contextual flags" {
    ...
    for ([_]spec.Shell{ .bash, .zsh, .fish }) |shell| {
        ...
        for ([_][]const u8{ "'monitoring:ui'", "'ui:metrics'", "'ui:logs'", "'ui:traces'", "'ui:grafana'", "'ui_logs:--open'", "'ui_logs:--no-open'", "'ui_metrics:--config'" }) |needle| try std.testing.expect(std.mem.indexOf(u8, output, needle) != null);
        for ([_][]const u8{ "'ui:alerts'", "'ui:log-alerts'", "'ui:alertmanager'", "'ui_alerts:--no-open'", "'ui_log_alerts:--no-open'", "'ui_alertmanager:--no-open'", "'ui_alerts:--config'", "'ui_log_alerts:--ssh-host'", "'ui_alertmanager:--config'" }) |needle| try std.testing.expect(std.mem.indexOf(u8, output, needle) != null);
        try std.testing.expect(std.mem.indexOf(u8, output, "'status:--open'") == null);
        try std.testing.expect(std.mem.indexOf(u8, output, "'status:--no-open'") == null);
