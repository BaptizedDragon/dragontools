...
pub const Command = enum { version, maintenance_check, app_apply, app_verify, app_status, install, verify, status, notify_test, ui_metrics, ui_logs, ui_traces, ui_grafana, ui_alerts, ui_log_alerts, ui_alertmanager, agents_install, agents_verify, agents_status, firewall, install_oh_my_zsh };
pub const Shell = enum { bash, zsh, fish };
pub const Node = enum { root, version, maintenance, maintenance_check, monitoring, app_apply, app_verify, app_status, install, verify, status, notify_test, ui, ui_metrics, ui_logs, ui_traces, ui_grafana, ui_alerts, ui_log_alerts, ui_alertmanager, agents, agents_install, agents_verify, agents_status, firewall, host, install_oh_my_zsh, completion, completion_bash, completion_zsh, completion_fish, wizard };
...
pub const commands = [_]CommandSpec{
    ...
    .{ .node = .ui, .parent = .monitoring, .name = "ui", .description = "Access investigation, dashboard and alerting UIs through an attached SSH tunnel" },
    .{ .node = .ui_metrics, .parent = .ui, .name = "metrics", .description = "Open a tunnel to VictoriaMetrics VMUI", .command = .ui_metrics },
    .{ .node = .ui_logs, .parent = .ui, .name = "logs", .description = "Open a tunnel to VictoriaLogs VMUI", .command = .ui_logs },
    .{ .node = .ui_traces, .parent = .ui, .name = "traces", .description = "Open a tunnel to VictoriaTraces VMUI", .command = .ui_traces },
    .{ .node = .ui_grafana, .parent = .ui, .name = "grafana", .description = "Open a tunnel to Grafana", .command = .ui_grafana },
    .{ .node = .ui_alerts, .parent = .ui, .name = "alerts", .description = "Open metrics/probe/host alert rule evaluation (vmalert metrics)", .command = .ui_alerts },
    .{ .node = .ui_log_alerts, .parent = .ui, .name = "log-alerts", .description = "Open log alert rule evaluation (vmalert logs)", .command = .ui_log_alerts },
    .{ .node = .ui_alertmanager, .parent = .ui, .name = "alertmanager", .description = "Open Alertmanager active alerts, silences and notification routing", .command = .ui_alertmanager },
...
const ui = &[_]Command{ .ui_metrics, .ui_logs, .ui_traces, .ui_grafana, .ui_alerts, .ui_log_alerts, .ui_alertmanager };
const all = &[_]Command{ .maintenance_check, .install, .verify, .status, .notify_test, .agents_install, .agents_verify, .agents_status, .firewall, .install_oh_my_zsh } ++ ui.*;
...
const native_ssh = &[_]Command{ .maintenance_check, .install, .verify, .status, .notify_test, .agents_install, .agents_verify, .agents_status, .install_oh_my_zsh } ++ ui.*;
const station = &[_]Command{.install};
const configured = &[_]Command{ .app_apply, .app_verify, .app_status, .install, .verify, .status, .notify_test } ++ ui.*;
...
pub const flags = [_]FlagSpec{
    .{ .name = "--no-open", .description = "Do not open a browser automatically", .kind = .boolean, .group = "UI", .commands = ui },
    .{ .name = "--open", .description = "Open the local URL in a browser (the default); retained for compatibility", .kind = .boolean, .group = "UI", .commands = ui },
...
pub fn uiCommand(command: Command) bool {
    return std.mem.indexOfScalar(Command, ui, command) != null;
}
pub fn stationCommand(command: Command) bool {
    return command == .install or command == .verify or command == .status or command == .notify_test or uiCommand(command);
}
...
const command_paths = paths: {
    // Generate every path from the growing catalog once, at compile time.
