...
pub fn render(a: std.mem.Allocator) ![]const u8 {
    return std.fmt.allocPrint(a,
        ...
        \\ExecStart=/opt/dragontools/components/victorialogs/current/victoria-logs-prod -storageDataPath=/var/lib/dragontools/victorialogs -httpListenAddr={s} -retentionPeriod={s} -retention.maxDiskUsagePercent={d}
        \\Restart=on-failure
        \\RestartSec=5s
        \\TimeoutStopSec=120s
        \\NoNewPrivileges=yes
        ...
        \\
    , .{
        @import("victorialogs.zig").listen_address,
