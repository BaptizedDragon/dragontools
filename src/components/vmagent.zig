...
pub const root = "/opt/dragontools/components/vmagent";
pub const pending = "/var/lib/dragontools/vmagent-restart-required";
pub const unit_path = "/etc/systemd/system/dragontools-vmagent.service";
pub const config_path = "/etc/dragontools/vmagent/prometheus.yml";
pub const data = "/var/lib/dragontools/vmagent";
...
// Reviewed 2026-09-17: official release API SHA256 digests.
...
pub fn artifact(arch: Arch) Artifact {
    return switch (arch) {
        .amd64 => .{
            ...
            .binary_sha256 = "a9fa98b7447b94f6bcf93b1c43fd3192b8f0cb2e8c54940af144692274bf4711",
        },
        .arm64 => .{
            .arch = "arm64",
            .archive_sha256 = "57c567b262962a4cb8e35c0c34efe64629a3e1ea69ac0611d8d67e168df8b1e8",
            .binary_sha256 = "da7046c7310c39ce3a93dc67f8f9562fa77a7fc9ec3d95b0cf8d9dcb6321679d",
        },
    };
}
...
pub fn binaryCommand(a: std.mem.Allocator, arch: Arch) ![]const u8 {
    ...
    return remote.shell(a, &.{
        ...
        \\test ! -L "$dest/vmagent-prod"
        \\if test -e "$dest/vmagent-prod"; then test -f "$dest/vmagent-prod" && test "$(stat -c '%h' "$dest/vmagent-prod")" = 1; fi
        ...
        \\    test -f "$root/$current_version/vmagent-prod" && test ! -L "$root/$current_version/vmagent-prod"
        \\  fi
        \\fi
        \\valid=0
        \\if test -f "$dest/vmagent-prod"; then
        \\  if printf '%s  %s\n' "$binary_hash" "$dest/vmagent-prod" | sha256sum --check --status; then valid=1; fi
        \\fi
        ...
        \\if test "$valid" = 1 && test "$current_version" = "$version" && test "$(stat -c '%u:%g:%a' "$root")" = 0:0:755 && test "$(stat -c '%u:%g:%a' "$dest")" = 0:0:755 && test "$(stat -c '%u:%g:%a' "$dest/vmagent-prod")" = 0:0:755; then printf unchanged; exit 0; fi
        ...
        \\  if test "$(stat -c '%u:%g' "$dest/vmagent-prod")" != 0:0; then chown root:root "$dest/vmagent-prod"; changed=1; fi
        \\  if test "$(stat -c '%a' "$dest/vmagent-prod")" != 755; then chmod 755 "$dest/vmagent-prod"; changed=1; fi
        \\fi
        \\tmp=''
        ...
        \\  test -f "$tmp/vmagent-prod" && test ! -L "$tmp/vmagent-prod"
        \\  printf '%s  %s\n' "$binary_hash" "$tmp/vmagent-prod" | sha256sum --check --status
        \\  install -o root -g root -m 755 "$tmp/vmagent-prod" "$tmp/binary.new"
        \\  test -f "$tmp/binary.new" && test ! -L "$tmp/binary.new"
        \\  mark_dirty
        \\  mv -fT "$tmp/binary.new" "$dest/vmagent-prod"
        \\  changed=1
        \\fi
        \\if test "$current_version" != "$version"; then
...
test "vmagent literal artifact pins cover both supported architectures" {
...
test "vmagent verifies archive before extracting only the expected regular binary" {
    ...
    for ([_]Arch{ .amd64, .arm64 }) |arch| {
        ...
        const binary_check = std.mem.indexOf(u8, command, "\"$binary_hash\" \"$tmp/vmagent-prod\" | sha256sum --check --status").?;
        ...
        try expectContains(command, "test -f \"$tmp/vmagent-prod\" && test ! -L \"$tmp/vmagent-prod\"");
...
test "vmagent rejects unexpected managed symlinks and preserves restart intent before atomic replacement" {
    const a = std.testing.allocator;
...
test "vmagent binary shell parses without executing changes" {
