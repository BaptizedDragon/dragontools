...
- Replace the reboot-required metric alert with persistent native host maintenance
  transition events. Add a restricted five-minute systemd observer with atomic
  private state, bounded/deduplicated package details, kernel and uptime. Unchanged
  state produces no event or state rewrite. Keep the reboot gauge for status.
- Ship host journal events through Vector and existing mTLS :9444 using a fixed
  trusted host stream, including when application logs are disabled. Verify timer,
  safe state and fresh stream read-only. Preserve independent activation intent.
- Add separate required/cleared VictoriaLogs alerts with annotation-only context
  and dedicated deduplicated Telegram routing without automatic resolved messages.
  Upgrade station install before application apply. Add cross-platform transition,
  safety, lifecycle and template tests plus an isolated pinned Linux event pipeline.


- Parse selected journald JSON objects into bounded searchable Vector log fields,
  preserving scalars and plain messages, normalizing level/time, and preventing
  application identity or VictoriaLogs control metadata injection. Prove the
  existing four trusted application stream fields during signal verification.
  Validate changed Vector candidates before replacing good configuration; preserve
  restart intent and Vector-only/no-op reruns. Add pinned Vector/VictoriaLogs
  query and base/app alert regression coverage, plus VMUI examples and explicit
  nested/array/collision policies. Historical logs, rules and ingress stay unchanged.

- Extend `monitoring ui` with `alerts`, `log-alerts` and `alertmanager` for private
  rule evaluation and routed-alert investigation. Reuse component ports, bounded
  readiness, automatic browser opening, `--no-open` and Ctrl-C cleanup. Add
  mapping/help/completion and Linux/macOS lifecycle coverage; keep service units,
  alert configuration and public exposure unchanged.

- Add `monitoring ui metrics|logs|traces|grafana` using station configuration and
  attached, strict OpenSSH tunnels. Prefer component ports, fall back on conflicts,
  bind only localhost, retry the native UI path for up to 10 seconds and clean up
  on Ctrl-C. Open the browser after readiness by default; `--no-open` suppresses
  it and the earlier `--open` remains compatible. Launcher failures are nonfatal;
  shared help/completion and Linux/macOS lifecycle tests cover the workflow.
  Favor native VMUI for investigation while
  retaining Grafana for dashboards; preserve authentication, ingestion and all
  remote service behavior.

- Add human-readable Telegram HTML notifications with distinct firing/recovery
  wording, application/service identity, severity symbols, guarded resolved
  durations and bounded grouped messages. Escape dynamic fields and omit raw logs
  and label dumps. Atomically manage the public template with independent
  Alertmanager restart intent and unchanged reruns. Keep resolved notifications,
  secret handling, routing and alert evaluation unchanged; test notifications
  clearly identify both test and completion. Add Linux/macOS HTML snapshots,
  pinned-native renderer coverage and publication/recovery/isolation fixtures.

- Fix application metrics readiness for multiple metric names sharing the same
  labels: retain names through timestamp transforms and return a bounded
  existence result. Add separate read-only source, activation, connectivity,
  mTLS, remote-write and visibility diagnostics, preserving down-target success,
  freshness, retry budgets and independent finalization. Extend the real pinned
  process fixture with histogram sum/count collisions and test recovery/no-op.
  No production agent configuration, routes, units or PKI changes.

- Fix healthy Caddy ingress failing verification when `systemctl show` renders
  `LoadCredential` as `[unprintable]`. Read the typed D-Bus credential ID/path
  mappings with `busctl --json=short` and compare them with the rendered unit;
  never read or print credential contents. Add separate Caddy account, binary,
  unit, configuration, directories, systemd, credentials, listener and TLS checks.
  Preserve exact mappings, hardening, units/configuration and independent restart
  intent. Add the observed Ubuntu production-state regression and recovery/no-op
  tests, including malformed/missing/extra credentials and diagnostic redaction.

- Fix ingress authorization's repeated restart/verification failure when systemd
  omits empty properties. Request `show --all`, compare effective hardening with
  the rendered unit, and keep missing/nonempty security properties as failures.
  Report separate safe account, unit, helper, directory, registry, server-state
  and systemd checks. Share registry/server modes with native PKI reconciliation;
  preserve units, private-key isolation and independent restart finalization.
  Add the observed production-state fixture, strict drift/redaction checks and
  retained-intent recovery followed by an unchanged rerun.
