...
ARTIFACTS = (
    ...
    ('alertmanager', 'alertmanager', 'prometheus/alertmanager', 'alertmanager-{bare}.linux-{arch}.tar.gz', 'alertmanager-{bare}.linux-{arch}/alertmanager'),
    ('alertmanager', 'amtool', 'prometheus/alertmanager', 'alertmanager-{bare}.linux-{arch}.tar.gz', 'alertmanager-{bare}.linux-{arch}/amtool'),
...
def fetch(directory, arch):
    ...
    for component, name, repo, archive, member in ARTIFACTS:
        ...
        binary_hash = re.search(r'\.' + ('amtool_sha256' if name == 'amtool' else 'binary_sha256') + r' = "([a-f0-9]{64})"', block)[1]
        path = directory / name
        if path.is_symlink():
    ...
    print('PASS: all nine Linux fixture binaries match production pins.')
