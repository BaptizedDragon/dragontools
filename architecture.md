...
`install/verify/status/notify-test/ui` load optional `./station.toml`, separately from
...
The default investigation interfaces are native VMUI; Grafana remains the
dashboard interface and a secondary investigation option. `monitoring ui` offers
`metrics`, `logs`, `traces`, `grafana`, `alerts`, `log-alerts` and `alertmanager`.
Each keeps an attached OpenSSH session and a loopback-only local forward.
The URL is printed after acknowledged forwarding
and bounded HTTP readiness retries; the browser then opens automatically unless
`--no-open` is set. Launcher failures keep the tunnel active; Ctrl-C closes it.
A private temporary control socket permits safe port-conflict retries without
reusing another SSH master or inheriting configured forwards. This is local
session ownership, with no daemon,
PID database or station mutation. Grafana authentication is unchanged; the command
never resolves secrets. Tracing-agent availability is independent of the stored
traces UI.

```text
LOCAL BROWSER -- 127.0.0.1:<chosen port> -- SSH --> STATION LOOPBACK
  metrics       --> VictoriaMetrics VMUI :8428/vmui/
  logs          --> VictoriaLogs VMUI    :9428/select/vmui/
  traces        --> VictoriaTraces VMUI  :10428/select/vmui/
  grafana       --> Grafana dashboards   :3000/
  alerts        --> vmalert-metrics      :8881/vmalert/groups
  log-alerts    --> vmalert-logs          :8880/vmalert/groups
  alertmanager  --> Alertmanager         :9093/
```

The two evaluator UIs show rule definitions, evaluation health and pending/firing
state. Alertmanager shows active/routed alerts, silences, grouping and notification
routing. All are accessed through SSH only; UI commands do not edit alerting
configuration, add public routing or change service listeners.
...
OTel traces agents, systemd-service state alerts, monitoring firewall,
...
checks; application-host agents and application dashboards remain owned by monitoring apply. Unsupported component paths and flags
...
0.32.0 is pinned independently of Grafana; managed dashboards use that datasource.
...
SecurityUpdatesPending. Every required rule must be healthy;
...
Telegram presentation is a separate managed HTML template, transported on public
stdin and atomically published as a root-owned `0644` file. Only Alertmanager's
restart intent changes when the template changes; verification compares exact
bytes read-only. Pinned Alertmanager's HTML renderer escapes dynamic labels.
State-aware single messages and bounded grouped titles use an explicit field
allowlist; annotations and raw log payloads never enter the message. No routing,
rule evaluation, secret transport or service topology changes are involved.
...
Dashboards have a separate versioned ownership manifest outside the native loaders.
VMUI reads direct `dragontools-app-<name>.json` files from its custom dashboard
path; Grafana's fixed file provider polls dedicated `DragonTools/<application>`
folders. Exact generated bytes and identity binding authorize reconciliation;
unmanaged files/UID collisions fail. Previous generations remain recognized until
both consumers show the desired dashboard. Dashboard publication never marks an
agent restart or a Victoria backend restart. There is no broad deletion/adoption.
HTTP mapping is excluded from target agent registrations, so query-only edits do
not alter Vector/vmagent configuration or credentials.

For each configured service, Vector's scheduled exec source invokes the native
helper every 15 seconds as `dt-vector`. The helper reads systemd's current
ControlGroup and bounded cgroup v2 files, and emits absolute metric events with
only trusted application/environment/host/service labels. No cgroup path or PID
becomes a stored label. The helper supplies a single normalized CPU/memory/tasks/
I/O/OOM contract instead of enabling Vector's static-path cgroup scan as well.
Existing Vector host collection and mTLS transport remain unchanged.

Grafana 13.2.2 uses its unified `resource` store for provisioned dashboards. The
read-only verifier selects only the expected dashboard UID and public provider,
source-path, panel and variable state; its empty legacy dashboard table is not
proof that a UID is free. SQLite writes and credential-column reads are denied.
...
Vector reads only configured journald units. Its fixed `agents/logs.vrl` parses
JSON objects, projects bounded top-level scalars without expanding nested objects
or arrays, normalizes timestamp/level/message, then attaches trusted
application/environment/host/service identity. The private ingress already sets
the VictoriaLogs `_stream_fields` ingestion parameter to precisely those four
fields for application registrations (host/service for the legacy agents path).
Application-provided underscore-prefixed fields cannot inject control metadata.
HTTP paths and other application scalars remain ordinary searchable fields;
historical records stay unchanged. Log verification uses an actual stream selector
and checks the returned `_stream` plus trusted fields and freshness. Vector
...
Vector configuration candidates pass the pinned binary's native validation before
atomic replacement; failed validation preserves the good file and prior intent.
...
scrape results (`up=0` is valid; `up=1` also requires recent real payload).
Timestamp transforms retain metric names so distinct application metrics with
identical labels cannot collide; an aggregate existence result bounds the reply.
Read-only app metrics diagnostics distinguish process activation, completed
scrapes (including down targets), TCP reachability, authenticated mTLS, successful
current-process remote-write requests and station visibility. Error counters need
not reset on recovery; successful requests alone cannot replace the final scoped
freshness check. Install/verify additionally require samples after
...
Station and application verification share the ingress managed checks. The unit
renderer supplies both exact file bytes and effective systemd expectations;
`systemctl show --all` retains empty capabilities/groups/drop-in properties and
an absent property fails closed. Named deterministic checks separate account,
unit, helper, directories, registry, server metadata and effective systemd state.
Registry/server directory modes come from the native PKI storage policy used for
publication, inspection and supported migration. The authorization verifier reads
only server-directory metadata; native PKI retains cryptographic validation.
Verification never clears restart intent: install finalizes each component only
after its runtime health succeeds.

Caddy's deterministic checks separate account, pinned binary, unit, config,
directories, effective hardening and credentials before bounded listener/TLS
readiness. `LoadCredential` is an `a(ss)` D-Bus property: human-readable
`systemctl show` output is not an encoding of those mappings. The verifier reads
the typed property through systemd's `busctl --system --json=short get-property`
and compares credential IDs/source paths with the rendered unit, independently
of ordering. Missing, extra, duplicate, malformed or incorrect mappings fail
`caddy_credentials`; no credential contents are requested. These checks are shared
by station and application verification and never finalize pending restart intent.
...
and 9444 (host events and selected logs) from monitored hosts; DragonTools performs no firewall mutation.
...
output, no shell and trusted labels. The existing vmalert host pack retains the
24-hour security-update warning. Reboot state remains a gauge but does not alert
from metrics. A separate restricted five-minute host-event timer observes reboot
transitions, persists tiny atomic state and writes JSON to journald. No
upgrade/refresh/reboot mutation or public port is added. Scheduled/manual CI reports
...
## Structured host maintenance transitions

```text
Monitored host
  dt-host-events timer (5m, no listener)
    native helper -> private atomic state + journald JSON
                                      |
  Vector :8686 loopback <-------------+
    bounded logs buffer -> mTLS :9444
Station
  Caddy :9444 -> private Unix socket authorization -> VictoriaLogs :9428
    vmalert-logs :8880 -> Alertmanager :9093 -> configured Telegram
```

All backend listeners above are loopback; only Caddy is public IPv4 ingress.
Stream identity is fixed to application=host, environment=host,
service=dragontools-host, with authenticated host=dt-<machine-id>. Package arrays
are searchable ordinary fields, encoded as one JSON value; packages, event and
kernel never become stream fields. Numeric host metrics retain their existing
:9443 path. Event context reaches Telegram as annotations, not metric labels.

Initial false persists silently; initial true emits one warning. Only subsequent
required/cleared transitions emit, and unchanged polls do not write. The observer
is application-host owned; station install owns shared authorization, rule packs
...
replayed journal timestamps stay historical. Long outages can suppress notification
while retaining the audit event; arbitrary crash/delivery exactly-once guarantees
are not claimed. No package management, automatic reboot, new transport, tracing
agent or general event framework is implemented.
