...
class Application(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        # Real applications expose multiple names with identical label sets (in
        # particular histogram _sum/_count). A one-metric fake hid a timestamp
        # query collision in application readiness.
        labels = '{host="forged-application",application="forged",environment="forged",service="forged"}'
        body = ('# TYPE fixture_requests_total counter\nfixture_requests_total' + labels + ' 7\n'
                '# TYPE fixture_duration_seconds histogram\nfixture_duration_seconds_sum' + labels + ' 3\n'
                'fixture_duration_seconds_count' + labels + ' 7\n'
                'fixture_duration_seconds_bucket' + labels[:-1] + ',le="+Inf"} 7\n').encode()
