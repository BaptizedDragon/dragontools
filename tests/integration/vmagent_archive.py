...
source = (Path(__file__).resolve().parents[2] / "src/components/vmagent.zig").read_text()
...
with tarfile.open(args.archive, "r:gz") as archive:
    ...
    for member in members:
        ...
        if member.name == "vmagent-prod":
            assert hashlib.sha256(archive.extractfile(member).read()).hexdigest() == hashes["binary_sha256"]
print("vmagent " + version + " Linux " + args.arch + ": archive, binary and safe entry catalog verified; no execution.")
