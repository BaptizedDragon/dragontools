...
def main(fixture):
    ...
    metrics_checks = load('metrics_checks', ROOT / 'src/monitoring/agents/metrics_checks.py')
    ...
    with tempfile.TemporaryDirectory(prefix='doers-runtime-') as temporary:
        ...
        try:
            ...
            start_host, end_host = vector_config.index('  host_events_journal:'), vector_config.index('  host_events_metadata:')
            vector_config = vector_config[:start_host] + '  host_events_journal:\n    type: demo_logs\n    format: shuffle\n    interval: 300\n    lines: [\'{"event":"host_stream_ready"}\']\n' + vector_config[end_host:]
            ...
            assert metrics_checks.check('metrics_source_ready', registration, vm_since)
            assert metrics_checks.check('metrics_station_reachable', registration, vm_since)
            assert metrics_checks.check('metrics_remote_write_accepted', registration, vm_since)
            selector = '{' + ','.join(k + '=' + json.dumps(v) for k, v in trusted.items()) + '}'
            assert query('fixture_requests_total' + selector)
            assert query('fixture_duration_seconds_sum' + selector)
            assert query('fixture_duration_seconds_count' + selector)
            # Production-shaped regression: all distinct names sharing a label
            # set are valid. The previous timestamp transform fails with 422;
            # exercise the actual pinned query engine, not a string-only fake.
            old_selector = selector[:-1] + ',agent="vmagent",__name__!~"up|scrape_.*"}'
            stamp = 'timestamp(' + old_selector + ')'
            old_query = '(' + stamp + ' >= ' + str(vm_since) + ') and (' + stamp + ' > time()-90)'
            connection = http.client.HTTPConnection('127.0.0.1', 8428, timeout=5)
            try:
                connection.request('POST', '/api/v1/query', urllib.parse.urlencode(dict(query=old_query)), {'Content-Type': 'application/x-www-form-urlencoded'})
                response = connection.getresponse()
                error = json.loads(response.read(16384))
                assert response.status == 422 and 'duplicate output timeseries' in error.get('error', '')
            finally:
                connection.close()
