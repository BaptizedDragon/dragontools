...
class Endpoint(unittest.TestCase):
    def run_check(self, scenario):
        functions = {'__name__': 'endpoint_fixture'}
        exec(compile((ROOT / 'src/monitoring/agents/endpoint.py').read_text(), 'endpoint.py', 'exec'), functions)
        calls = []
        def addresses(*_):
            calls.append('dns')
            ipv4 = (socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP, '', ('127.0.0.1', 9443))
            ipv6 = (socket.AF_INET6, socket.SOCK_STREAM, socket.IPPROTO_TCP, '', ('::1', 9443, 0, 0))
            return None if scenario == 'dns' else [ipv6, ipv4] if scenario.startswith('dual-') else [ipv4]
        functions['_endpoint_dns'] = addresses
        class Socket:
            def settimeout(self, timeout):
                assert 0 < timeout <= 4
            def connect(self, address):
                calls.append(('tcp-v4' if address[0] == '127.0.0.1' else 'tcp-v6') if scenario.startswith('dual-') else 'tcp')
                if address[0] == '::1':
                    assert scenario == 'dual-fallback', 'unreachable IPv6 must not precede the managed IPv4 listener'
                else:
                    assert address == ('127.0.0.1', 9443)
                    if scenario == 'dual-fallback':
                        raise ConnectionRefusedError('PRIVATE IPv4 diagnostic')
                if scenario == 'tcp':
                    raise ConnectionRefusedError('PRIVATE network diagnostic')
            def close(self):
                pass
        class Context:
            def load_cert_chain(self, certificate, key):
                calls.append('client-files')
                assert certificate == '/managed/client.crt' and key == '/managed/client.key'
                if scenario == 'missing-key':
                    raise ValueError('PRIVATE key diagnostics')
            def wrap_socket(self, connection, server_hostname):
                calls.append('tls')
                assert self.check_hostname and self.verify_mode == ssl.CERT_REQUIRED
                assert server_hostname == 'station.example'
                if scenario == 'server-tls':
                    raise ssl.SSLCertVerificationError(1, 'PRIVATE hostname certificate mismatch')
                if scenario == 'client-tls':
                    error = ssl.SSLError(1, 'PRIVATE client rejection')
                    error.reason = 'SSLV3_ALERT_BAD_CERTIFICATE'
                    raise error
                return connection
        def context(purpose, cafile):
            calls.append('server-ca')
            assert purpose == ssl.Purpose.SERVER_AUTH and cafile == '/managed/ca.crt'
            return Context()
        class Request:
            def __init__(self, host, port, timeout):
                assert (host, port, timeout) == ('station.example', 9443, 4)
            def request(self, method, route):
                calls.append('health')
                assert (method, route) == ('GET', '/health')
                if scenario == 'tls13-client':
                    error = ssl.SSLError(1, 'PRIVATE TLS 1.3 deferred client rejection')
                    error.reason = 'TLSV1_ALERT_UNKNOWN_CA'
                    raise error
            def getresponse(self):
                status = 403 if scenario == 'unregistered' else 503 if scenario == 'ingestion' else 204
                return types.SimpleNamespace(status=status, read=lambda limit: b'')
            def close(self):
                pass
        stdout, stderr = io.StringIO(), io.StringIO()
        with patch('socket.socket', return_value=Socket()), patch('ssl.create_default_context', side_effect=context), \
             patch('http.client.HTTPConnection', Request), contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            code = functions['check']('station.example', '/managed')
        self.assertEqual(stdout.getvalue(), '')
        self.assertEqual(stderr.getvalue(), '')
        return code, calls

    def test_dns_tcp_tls_authentication_and_ingestion_have_distinct_silent_codes(self):
        for scenario, code in [('dns', 91), ('tcp', 92), ('server-tls', 93), ('missing-key', 94),
                               ('client-tls', 94), ('tls13-client', 94), ('unregistered', 94), ('ingestion', 95), ('ready', 0)]:
            with self.subTest(scenario=scenario):
                self.assertEqual(self.run_check(scenario)[0], code)

    def test_diagnostics_stop_in_order_without_later_network_or_credential_operations(self):
        self.assertEqual(self.run_check('dns')[1], ['dns'])
        self.assertEqual(self.run_check('tcp')[1], ['dns', 'tcp'])
        self.assertEqual(self.run_check('server-tls')[1], ['dns', 'tcp', 'server-ca', 'client-files', 'tls'])
        self.assertEqual(self.run_check('ready')[1], ['dns', 'tcp', 'server-ca', 'client-files', 'tls', 'health'])

    def test_mixed_dns_prefers_managed_ipv4_listener_with_bounded_family_fallback(self):
        self.assertEqual(self.run_check('dual-ready'), (0, ['dns', 'tcp-v4', 'server-ca', 'client-files', 'tls', 'health']))
        self.assertEqual(self.run_check('dual-fallback'), (0, ['dns', 'tcp-v4', 'tcp-v6', 'server-ca', 'client-files', 'tls', 'health']))

    def test_resolver_error_is_bounded_and_never_prints_exception(self):
        functions = {'__name__': 'endpoint_fixture'}
        exec(compile((ROOT / 'src/monitoring/agents/endpoint.py').read_text(), 'endpoint.py', 'exec'), functions)
        stdout, stderr = io.StringIO(), io.StringIO()
        with patch('socket.getaddrinfo', side_effect=socket.gaierror(socket.EAI_NONAME, 'PRIVATE resolver diagnostic')), \
             contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            self.assertIsNone(functions['_endpoint_dns']('station.example', 9443))
        self.assertEqual(stdout.getvalue(), '')
        self.assertEqual(stderr.getvalue(), '')

    def test_service_endpoint_preserves_semantic_code_after_runtime_guard(self):
        functions = {'__name__': 'checks_fixture'}
        exec(CHECKS, functions)
        calls = []
        functions['runtime'] = lambda spec: calls.append('runtime')
        def diagnostic(host, path):
            calls.append('endpoint')
            self.assertEqual((host, path), ('station.example', '/etc/dragontools/vector'))
            return 94
        functions['check'] = diagnostic
        with self.assertRaises(SystemExit) as failure:
            functions['endpoint'](dict(station='station.example', kind='vector'))
        self.assertEqual(failure.exception.code, 94)
        self.assertEqual(calls, ['runtime', 'endpoint'])
