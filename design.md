...
## Attached monitoring UI access

`monitoring ui` serves `metrics`, `logs`, `traces`, `grafana`, `alerts`, `log-alerts`
and `alertmanager`. All reuse the station parser and connection precedence,
...
[VictoriaLogs v1.52.0 `/select/vmui/`](https://github.com/VictoriaMetrics/VictoriaLogs/blob/v1.52.0/app/vlselect/main.go), and
[VictoriaTraces v0.11.0 `/select/vmui/`](https://github.com/VictoriaMetrics/VictoriaTraces/blob/v0.11.0/app/vtselect/main.go).
Trailing slashes select the native HTML entrypoints directly. Grafana's root
retains its normal login redirect and authentication; no credential is resolved.
The existing [vmalert v1.152.0 handler](https://github.com/VictoriaMetrics/VictoriaMetrics/blob/v1.152.0/app/vmalert/web.go)
serves `/vmalert/groups` for rule definitions and evaluation (metrics/probes/hosts
on 8881, logs on 8880). The [Alertmanager v0.34.1 UI handler](https://github.com/prometheus/alertmanager/blob/v0.34.1/ui/web.go)
serves `/` for its native active alerts/silences/grouping/routing interface on 9093.
The shared component port definitions also feed service-unit generation; rendered
unit content remains unchanged. All seven commands use the same tunnel/readiness/
opener path. Access adds no DragonTools rule editor, notification mutation,
Grafana alerting, public reverse proxy or authentication proxy.

An argv-spawned OpenSSH master remains attached, with strict host keys, native
alias/ProxyJump/IdentityAgent configuration, no agent forwarding, no remote
command and no persistent/background master. `ClearAllForwardings=yes` prevents
user SSH configuration from opening additional listeners. Because that option
also clears command-line `-L`, the controller adds its one explicit forward via
the master's private control socket (`ssh -O forward`). Only these local mux
requests use isolated config, never the authenticated master. A mode-0700 random
temporary directory protects the socket and is removed on normal/error/signal
exit; no PID file or registry persists.

The controller probes an exclusive `127.0.0.1` bind, preferring the component port
then asking the OS for a free ephemeral port. The socket is released for OpenSSH;
its synchronous acknowledgement, rather than a port-connect guess, proves forward
ownership. A demonstrated bind race retries with another port (at most four
attempts); denied forwarding fails semantically. A read-only GET of the fixed UI
path checks for HTTP 2xx/3xx before printing the local URL. It does not follow
redirects or expose response contents. HTTP failures retry after 150 ms and then
500 ms, within a 10-second overall readiness deadline. Each cancellable attempt
has at most two seconds (or the remaining overall budget) for connect/write/read;
Zig 0.16's unimplemented POSIX per-connect timeout option is not used. No browser
opens until an attempt succeeds. SSH startup separately allows approximately
30 seconds; authentication/forward failures suppress raw SSH stderr.

SIGINT/SIGTERM set a cancellation flag; the owner terminates/reaps its private SSH
process group, with a bounded TERM-to-KILL fallback, then removes its socket.
Unexpected SSH exit, including during HTTP readiness, fails and suggests
reconnecting. Browser opening is the default: `open`/`xdg-open` receives only the
generated URL by argv after readiness. `--no-open` skips the launcher. Unavailable,
unsupported, failed or timed out launchers print a manual URL and keep the tunnel
running. The original explicit `--open` remains compatible; combining it with
`--no-open` is rejected. No GUI is required by the local fixture.
Local tests cover station-config precedence, endpoint mapping, strict SSH argv,
port conflicts/races, HTTP/auth failures, cancellation, cleanup and browser failure
on Linux/macOS. These fixtures are not a supported-host or browser-rendering claim.
...
Vector 0.58.0 reads only selected journald units, parses JSON objects using fixed
native VRL, and attaches trusted identity after parsing. Plain, malformed and
non-object JSON messages remain intact. Numeric/boolean top-level values retain
their types. Common log/HTTP fields and at most 64 additional scalars (32 KiB
additional value budget, 8 KiB per promoted string) have bounded ASCII field
names; nested objects/arrays/nulls are not promoted. A useful explicit `message`
is preferred, otherwise the original JSON is kept; no duplicate original payload
is created. Reserved identity/type/journal keys and underscore-prefixed Victoria
control fields are excluded. Levels normalize to lowercase; absent levels use
only an explicitly present valid journal priority. RFC3339 app timestamps are
accepted within 24 hours of the journal timestamp; invalid/outlying ones fall
back. README documents this policy and searchable examples.

The existing private ingestion route supplies `_time_field=timestamp`,
`_msg_field=message` and exactly the trusted application/environment/host/service
`_stream_fields` (legacy agents retain host/service). Verification requires actual
stream identity and fresh fields, accepts quiet metadata and never assumes every
service emits HTTP logs. Host metrics have a separate transform and do not pass
through log parsing. Historical records are not migrated, and rule definitions,
transport ports, Caddy and ingress authorization remain unchanged. Vector uses
...
finalization. Changed Vector YAML is staged alongside the destination and natively
validated as `dt-vector` with explicit `--config-yaml` before marking restart
intent and atomically publishing it. Validation failure removes only the candidate,
preserving the good config and any prior intent; unchanged content is never
rewritten. Vector-only changes do not restart vmagent; target-only changes do
...
Application payload readiness uses `timestamp(...) keep_metric_names` before its
post-start/90-second freshness filters. Without preserved names, ordinary series
such as histogram `_sum`/`_count` collide when all other labels match and the
pinned VictoriaMetrics query API returns 422. `count(...) > 0` bounds successful
responses to one existence value without choosing an application-specific metric.
The read-only controller separately reports metrics source/agent/reachability,
mTLS, remote-write acceptance and station visibility. Completed failed scrapes
remain valid monitoring data. No unit, scrape config, route, credential or retry
deadline changes are needed for this query correction.
...
Ingress managed verification reports fixed `managed_account`, `managed_unit`,
`managed_helper`, `managed_directories`, `managed_registry`, `managed_server_state`
and `managed_systemd_properties` checks. It requests all systemd properties so an
explicitly empty capability set is distinguishable from an absent property.
Expected hardening comes from the installed unit renderer, with set comparison
only for unordered list properties. Native PKI and the verifier share directory
mode constants; no modes or service access rights change for diagnostic purposes.
The production-state fixture uses observed systemd values and unequal uid/gid,
and runs the generated activation/finalization scripts against local fakes to
prove pending restart recovery and the next no-op.

Caddy has separate `caddy_account`, `caddy_binary`, `caddy_unit`, `caddy_config`,
`caddy_directories`, `caddy_systemd_properties`, `caddy_credentials`,
`caddy_listener` and `caddy_tls` checks. Its credential check reads only the
`org.freedesktop.systemd1.Service.LoadCredential` typed `a(ss)` property through
`busctl --system --json=short`. `systemctl show --all` can print `[unprintable]`
for this structured property even when Caddy is healthy. The typed response must
contain each generated ID/source-path pair exactly once and no other mappings;
response order is immaterial. There is no display-string fallback, ignored
property or OS-specific exception. Command/JSON/shape failures are deterministic
and suppress raw output. Expected mappings still derive from the same generated
unit used by installation, and successful runtime verification gates finalization.
...
TCP 9444 for host events and selected logs from monitored hosts and retain their SSH policy.
...
make TCP 9443 (metrics) and 9444 (host events and selected logs) reachable and manage DNS/provider firewalls themselves. No provider
...
Notification presentation lives in the DragonTools-managed
`/etc/dragontools/alertmanager/templates/telegram.tmpl`, root:root `0644` in a
`0755` directory. Public template bytes travel on stdin, not in argv. Publication
refuses foreign files, symlinks, hardlinks and incompatible metadata, preserves
unrelated templates, writes restart intent before atomic replacement and clears
it only after normal Alertmanager verification. Identical bytes are a no-op.
The enabled receiver explicitly selects this template with `parse_mode: HTML`
and `send_resolved: true`; grouping, timing, routing and rule definitions stay fixed.

Pinned Alertmanager v0.34.1 executes HTML templates with contextual escaping;
the managed template never uses `safeHtml` or `safeUrl`. Titles prefer application,
service, probe, host, then alert name. Critical/warning/neutral firing markers are
🚨/⚠️/ℹ️; recovery always uses ✅ and describes the cleared condition. For example,
“🚨 Doers unavailable” becomes “✅ Doers recovered”. Resolved start/end timestamps
must be nonzero and increasing; their difference is rounded to seconds and shown
in compact Go duration notation, e.g. `1h15m0s`. No external state is required.

Only deliberate identity, probe target, filesystem and severity fields are shown.
No arbitrary annotations, raw logs or internal label maps enter the template.
Current host rules expose no numeric usage/threshold annotations, so those fields
are omitted; log messages refer to the configured threshold without inventing a
count. Display identity is capped at 48 Unicode characters, ordinary fields at
64, target at 256 and severity at 12, with control characters replaced by spaces.
Groups show at most ten 48-character identities and a remaining count, preserving
each alert's firing/resolved wording even in mixed groups. These limits account
for HTML entity expansion and keep rendered output below Telegram's 4096-character
limit. Tests cover snapshots, adversarial expansion and balanced markup with Go's
standard library on Linux/macOS; `tests/telegram_template_test.py --amtool PATH`
also validates the pinned native renderer and generated receiver configuration.
...
alerts independently while install/verify runs. The test uses the same template,
with an informational “DragonTools notification test” title; its existing resolved
delivery is retained as “DragonTools notification test completed”, never recovery
from an outage. Local tests cover redaction,
...
SecurityUpdatesPending is rendered in the host pack using the verified native-agent/Vector
metric contract and a 24-hour warning delay. RebootRequired is removed from this
pack; reboot transitions use structured host events.
...
are run. The implemented SecurityUpdatesPending warning waits 24 hours. The restricted
host-event timer observes reboot transitions every five minutes without a listener
or arbitrary remote command channel. Future explicit upgrades must verify new artifacts,
...
## Reboot transitions: events carry rich context

Continuous numerical state remains in metrics. Discrete reboot-required/cleared
transitions are `HostEvent` values emitted by the existing native helper. There
is no generic event framework or new upstream component. The observer service uses
dt-host-events, no capabilities, a read-only system, private devices/tmp and only
one writable 0700 state directory. A flock serializes checks, inputs are bounded,
unsafe links/metadata fail before output, and 0600 state is fsynced and renamed.
Unchanged successful checks neither emit nor rewrite. Initial true emits; initial
false only saves state. Output precedes commit so failure can replay an identical
host/sequence event ID rather than silently dropping a transition. This is not a
transaction across journald and disk, and state must not be reset casually.

Vector reads only the dedicated host service journal, preserves its timestamp and
uses a separate transform from application JSON. Its reserved type and fixed
application=host/environment=host/service=dragontools-host labels are normalized
again at the authenticated ingress; host always comes from registered identity.
The fixed 20-package array is stored as a JSON field with the full unique count;
Debian package-name validation excludes arbitrary text. Kernel and uptime come
from /proc; human duration is presentation only. No apt commands run here.

The shared vlogs group evaluates each minute, selects two minutes of required or
cleared events and aggregates only by host/event_id. Annotation-only query calls
retrieve package/kernel/uptime fields. This follows the separation described in
[VictoriaLogs annotation query guidance](https://docs.victoriametrics.com/victorialogs/vmalert/#how-to-attach-a-sample-log-row-to-alerts).
The pinned v1.52.0 stats API returns row_any as JSON, so fixed field-specific
annotation queries group their temporary result by that field; those results are
never stored as ALERTS series. The primary alert query contains no rich fields.

A first Alertmanager child route groups by alertname/host/event_id, sends immediately,
uses a one-minute group interval and 24-hour repeat interval, and disables automatic
resolved messages. The short-lived explicit clear alert produces the recovery-style
message. Other receiver behavior and secrets are unchanged. HTML escaping and size
bounds apply to details. An outage beyond the event window may prevent notification;
this feature does not backfill notifications from old audit history.

Station install upgrades authorization, shared rules and the template before app
apply upgrades the host helper, Vector and observer units. Apply owns independent
...
Linux fixture runs pinned Vector, VictoriaLogs, vmalert and Alertmanager with local
webhook delivery and synthetic journal/peer inputs. It checks parsed fields,
annotation-only context, time-window expiry and notification deduplication. It is
not a systemd, real two-host TLS or human Telegram delivery claim.
